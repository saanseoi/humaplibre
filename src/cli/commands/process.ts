...
import { createDefaultManifest, saveManifest } from "../../domain/manifest.ts";
import {
...
export async function runProcessCommand(argv: string[]): Promise<void> {
	...
	const sourceProject = projectFlag
		? importProjects.find(
				(project) =>
					project.sourceName === projectFlag ||
					project.outputProjectName === projectFlag,
			)
		: undefined;
	...
	const selectedProject =
		sourceProject ??
		(await promptHumapProjectSelection(
			importProjects.map((project) => project.sourceName),
		));
	const importProject =
		sourceProject ??
		importProjects.find((project) => project.sourceName === selectedProject);

	if (!importProject) {
		throw new CliError(
			`Unknown Humap project: ${projectFlag ?? selectedProject}`,
		);
	}
	...
	const results = await exportHumapCollections(
		importProject,
	...
	summary.records = results.reduce(
		(count, result) => count + result.collection.features.length,
		0,
	);
	summary.images = results.reduce(
		(count, result) => count + result.imagesCopied,
		0,
	);
